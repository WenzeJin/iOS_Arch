...
class BooksView4: UITableViewController, WKUIDelegate{
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if difficulty == "M" && self.vol == 1 {
            let vc = UIStoryboard(name: "Main", bundle:nil).instantiateViewController(withIdentifier: "gvc") as! GameViewController
            vc.jsonName = String(format: "INKY_4M_b%@_4pp", numberFormatter.string(from: num)!)
            vc.imgName = String(format: "INKY_4M_b%@_4pp", numberFormatter.string(from: num)!)
            vc.loadViewIfNeeded()
            vc.loadGame()
